"""
S&P 500 Current-only Risk-State Shadow Evaluator
================================================

Absolute future validation of the frozen Current S&P 500 Regime score.
...
LOG_PATH = Path(
    "shadow_logs/sp500_shadow_log.csv"
)

OUT_SUMMARY = Path(
    "shadow_logs/sp500_shadow_evaluation_summary.csv"
)

OUT_BLOCKS = Path(
    "shadow_logs/sp500_shadow_time_blocks.csv"
)

OUT_PHASE = Path(
    "shadow_logs/sp500_shadow_phase_auc.csv"
)

OUT_EPISODES = Path(
    "shadow_logs/sp500_shadow_stress_episodes.csv"
)

OUT_GATE = Path(
    "shadow_logs/sp500_shadow_gate.csv"
)

OUT_HISTORY = Path(
    "shadow_logs/sp500_shadow_evaluation_history.csv"
)

OUT_REPORT = Path(
    "shadow_logs/sp500_shadow_report.md"
)
...
# Evaluation stages use matured 20D outcomes because Risk-State is defined
# against the next 20 trading days.
MIN_20D_EARLY = 60
MIN_20D_INTERIM = 125
MIN_20D_FORMAL = 252
...
def _stress_auc(
    ...
    # Lower regime score = higher stress probability.
    risk = -frame[
        "score"
...
def _stress_episodes(
    ...
    """
    Descriptive early-warning diagnostic.

    A stress episode begins when Stress_Event_20D changes from 0 to 1.
    For each onset, inspect the prior 20 eligible observations and record
    whether Current had already fallen to <=40.
    """
    ordered = sample.sort_values(
        "observation_date"
    ).reset_index(
...
def main():
    ...
    if stage == "COLLECTING":
        verdict = (
            "NO_DECISION_COLLECTING"
        )

    elif stage == "EARLY_READ":
        verdict = (
            "NO_DECISION_EARLY_READ"
        )

    elif model_gates.empty:
        verdict = (
            "NO_DECISION_INSUFFICIENT_GATES"
        )
    ...
    print(
        "S&P 500 Current-only Risk-State evaluator completed."
    )
    print(
        f"Stage: {stage}"
    )
