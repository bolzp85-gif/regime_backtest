"""
S&P 500 Current-only Risk-State Shadow Logger
=============================================

Frozen on 2026-09-11.
First eligible calendar date: 2026-09-12.

Because 2026-09-12 is a Saturday, the first actual S&P 500 daily market
observation is expected on the next trading day.
...
MODEL_VERSION = (
    "SP500_CURRENT_RISK_STATE_FROZEN_2026-09-11_v1"
)

PIPELINE_VERSION = (
    "ResearchLab_v1.0.17_PIT"
)

EXPECTED_CORE_SHA256 = (
    "936f91b579fabccfb3842911b919f2fcfb1de6bb8c601e20a6ae508357680297"
)
...
FROZEN_CONFIG_SNAPSHOT = {'pillar_weights': {'Makroökonomie': 0.25, 'Positionierung': 0.15, 'Marktinterna': 0.2, 'Technischer_Trend': 0.2, 'Fundamentale_Faktoren': 0.1, 'Fruehwarnindikatoren': 0.1}, 'sub_weights': {'Makroökonomie': {'fed_policy': 0.2, 'real_yields': 0.3, 'usd_index': 0.2, 'net_liquidity': 0.3}, 'Positionierung': {'cot_noncommercials': 0.5, 'fear_greed': 0.5}, 'Marktinterna': {'market_momentum': 0.5, 'vix_score': 0.5}, 'Technischer_Trend': {'distance_200ma': 0.35, 'distance_50ma': 0.35, 'rsi_momentum': 0.3}, 'Fundamentale_Faktoren': {'pe_valuation': 1.0}, 'Fruehwarnindikatoren': {'credit_spreads': 0.5, 'move_index': 0.3, 'vvix_score': 0.2}}}

MIN_COVERAGE = 60.0
HISTORY_YEARS = 15

# Avoid logging an unfinished US daily bar. 22:00 UTC is safely after the
# regular cash close even outside US daylight-saving time.
...
LOG_PATH = (
    LOG_DIR
    / "sp500_shadow_log.csv"
)

SUMMARY_PATH = (
    LOG_DIR
    / "sp500_shadow_summary.csv"
)
...
def _assert_frozen_pipeline():
    ...
    if (
        ...
        raise RuntimeError(
            "Frozen S&P 500 shadow core changed. "
    ...
    if not _configs_semantically_equal(
        ...
        raise RuntimeError(
            "Frozen S&P 500 Current configuration changed. "
...
def _calculate_frames():
    ...
    if raw.empty:
        raise RuntimeError(
            "S&P 500 research dataset is empty."
        )
...
def _append_missing_observations(
    ...
    if common.empty:
        raise RuntimeError(
            "No common S&P 500 Current observation is available."
        )
...
def main():
    ...
    print(
        "S&P 500 Current-only Risk-State shadow logger completed."
    )
    print(
        f"Rows: {len(log_df)}"
    )
