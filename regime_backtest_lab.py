...
- Main tests:
    ...
    * Forward max drawdown / realized volatility
    * Stress-event AUC
    * Rolling IC
...
FRED_FALLBACK_LAGS = {
    ...
    "WCESTUS1": 5,    # week-ending Friday -> regular Wednesday release
}
...
def fetch_yahoo_research_bundle(
    ...
    if (
        ...
        return (
            pd.DataFrame(),
            {
                "Yahoo": (
                    False,
                    "Keine Yahoo-Historie geladen."
                )
            },
        )

    close = flatten_yf_field(
        data,
        "Close"
    )

    volume = flatten_yf_field(
        data,
        "Volume"
    )
    ...
    for alias, source_col in [
        ...
        if source_col in close.columns:
            df[alias] = pit_reindex(
                close[source_col],
                df.index
            )
        else:
...
def _fred_first_release_available_series(
    ...
    Expected get_series_all_releases() columns in fredapi:
    date, realtime_start, value.

    If several observations first become available on the same day,
    the latest observation date is used as the current level.
    """
    if not hasattr(
        fred,
        ...
        raise AttributeError(
            "fredapi has no get_series_all_releases"
        )
    ...
    if releases.empty:
        raise ValueError(
            "Keine verwertbaren ALFRED-Releases"
        )

    # First release per observation.
    first = (
        releases
        .sort_values(
            [
    ...
    # On any publication date, use the newest observation released.
    first = (
        first
        .sort_values(
            [
...
if passed_count == len(
    ...
    st.success(
        "🟢 Alle vorab definierten Kernkriterien sind erfüllt. "
        "Das wäre ein starkes Signal, Literature Prior v1 in einem "
        "weiteren Walk-Forward-/Stabilitätstest zu validieren."
    )

elif passed_count >= 3:
    st.warning(
        f"🟡 {passed_count}/{len(rules)} Kriterien erfüllt. "
        "Interessantes Ergebnis, aber noch keine ausreichende Grundlage "
        "für eine produktive Gewichtsänderung."
    )

else:
    st.error(
        f"🔴 Nur {passed_count}/{len(rules)} Kriterien erfüllt. "
        "Die Literature Prior sollte auf Basis dieses Samples nicht "
        "automatisch in Produktion übernommen werden."
    )
...
- FRED First-Release/Vintage wird bevorzugt;
